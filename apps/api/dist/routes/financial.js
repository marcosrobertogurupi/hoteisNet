...
async function financialRoutes(server) {
    // Corporate Invoice Creation on Checkout
    server.post("/api/financial/corporate-invoice", async (request, reply) => {
        const { companyId, stayCheckinId, totalAmount, paymentTermsDays } = request.body;
    ...
    server.post("/api/financial/cash-bleed", async (request, reply) => {
        const { cashRegisterId, amount, operatorName, reason } = request.body;
    ...
    server.post("/api/financial/emit-invoice", async (request, reply) => {
        const { invoiceType, recipientCnpjCpf, totalValue, items } = request.body;
        return reply.status(201).send({
            success: true,
