...
// ============================================================
// In-memory Database Store for Hóspedes (HospedesNet)
// ============================================================
export interface GuestRecord {
  ...
  whatsappName?: string;
  hasWhatsapp?: boolean;
  telefonesVerificados?: VerifiedPhoneRecord[];
  emails: string[];
  primaryEmail: string;
  ...
  autoCreatedFromHub: boolean;
  createdAt: string;
}

const GUESTS_DATABASE: Map<string, GuestRecord> = new Map();
...
// ============================================================
// Hub do Desenvolvedor API token & contract (embedded per user request)
// ============================================================
const HUB_API_TOKEN = "183262310hxRtwiDQAo330874544";
const HUB_API_CONTRACT = "c2NqUUo0bFBLYzhuRmhrUWtvMXhUcjg4ZHFiTitCK1hBT3M4TDlRenllVT0=";
 ...
 */
export function verifyAllPhonesUazapi(telefones: string[], guestFullName: string): VerifiedPhoneRecord[] {
...
export function verifyUazapiWhatsapp(telefones: string[], guestFullName: string) {
...
// Stay Routes
// ============================================================
export async function stayRoutes(server: FastifyInstance) {
  ...
  server.post("/api/stay/verify-whatsapp", async (request, reply) => {
    const { phone, guestName } = request.body as { phone: string; guestName?: string };
  ...
  server.get("/api/stay/hub-consult-cpf", async (request, reply) => {
    ...
    try {
      const hubUrl = `https://ws.hubdodesenvolvedor.com.br/v2/cadastropf/?cpf=${cleanCpf}&token=${HUB_API_TOKEN}&contract=${HUB_API_CONTRACT}`;
      ...
      if (data && (data.status === true || data.return === "OK") && data.result) {
        ...
        const telefonesList: string[] = (res.listaTelefones || [])
          .map((t: any) => t.telefoneComDDD)
          .filter(Boolean);
        const primaryPhone = telefonesList[0] || "";

        const emailsList: string[] = (res.listaEmails || [])
          .map((e: any) => e.enderecoEmail)
          .filter(Boolean);
        ...
        const fallbackGuest: GuestRecord = {
          id: `HOSP-${cleanCpf}`,
  ...
  server.post("/api/stay/consumption", async (request, reply) => {
    const { stayCheckinId, roomId, items } = request.body as any;
  ...
  server.patch("/api/stay/room-status", async (request, reply) => {
    const { roomId, newStatus, housekeeperName } = request.body as any;
  ...
  server.patch("/api/stay/change-period", async (request, reply) => {
    const { stayId, roomId, checkInDate, newCheckOutDate, ratePerNight, tariffName } =
      request.body as any;
