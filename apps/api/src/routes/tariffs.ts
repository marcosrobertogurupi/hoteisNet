...
export interface TariffItem {
  id: string;
  name: string;      // Tarifas (ex: "APTO ESPECIAL INDIVIDUAL")
  adults: number;    // Qtd.Adulto (ex: 1, 2, 3)
  price: number;     // Valor apto (ex: 170.00)
}

// Initial mock dataset matching Print 3
let MOCK_TARIFFS: TariffItem[] = [
...
export async function tariffRoutes(server: FastifyInstance) {
  ...
  server.post("/api/tariffs", async (request, reply) => {
    const { name, adults, price } = request.body as any;
