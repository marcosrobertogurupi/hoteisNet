...
export async function stockRoutes(server: FastifyInstance) {
  ...
  server.post("/api/stock/transfer", async (request, reply) => {
    const { productId, toPosLocationId, quantity, transferredBy } = request.body as any;
  ...
  server.post("/api/stock/pos-deduction", async (request, reply) => {
    const { posLocationId, productId, quantitySold, saleSource } = request.body as any;
