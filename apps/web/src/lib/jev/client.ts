// Cliente HTTP do Jev (TypeSafe) via OpenRouter — Decisions API.
// Jev é um modelo de DECISÃO: não escreve texto; responde Choice/Score/Noul com probabilidades.
// Docs: https://openrouter.ai/docs/guides/community/jev  |  https://docs.typesafe.ai/llms.txt
//
// Nunca lança: qualquer falha (sem chave, timeout, HTTP ≠ 2xx, JSON inválido) volta como
// { ok: false, error } para o chamador cair no fluxo atual. A chave vem só de
// process.env.OPENROUTER_API_KEY (CLAUDE.md §6 — nunca hardcoded).
//
// Espelhado em apps/worker/src/jev.ts (o worker é CJS puro e não importa apps/web).
  ...
  try {
    const response = await fetch(JEV_ENDPOINT, {
      ...
      body: JSON.stringify({
        ...
        // Dados de hóspedes: só provedores sem retenção (a TypeSafe está na lista ZDR do OpenRouter).
    ...
    if (!json?.answers || typeof json.answers !== "object") {
      return { ok: false, error: "Resposta do Jev sem answers.", durationMs };
    }
    return {
      ok: true,
    ...
    return { ok: false, error: (err as Error)?.message || String(err), durationMs: Date.now() - startedAt };
  }
}
