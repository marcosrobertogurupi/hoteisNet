...
import { ShieldCheck, Plus, Edit3, Power, ArrowLeft, X, Loader2 } from "lucide-react";
...
export default function UsuariosPage() {
  ...
  const [form, setForm] = useState({ name: "", email: "", password: "", role: "RECEPCIONIST" });
  ...
  const openCreate = () => {
    setEditing(null);
    setForm({ name: "", email: "", password: "", role: "RECEPCIONIST" });
    setShowModal(true);
  };
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 bg-slate-900/80 p-6 rounded-3xl border border-slate-800 shadow-xl">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">Contas de login (e-mail/senha), papéis de acesso e auditoria.</p>
            </div>
          </div>
          <button
            onClick={openCreate}
        ...
        <div className="bg-slate-900/80 border border-slate-800 rounded-3xl overflow-hidden shadow-2xl">
          <table className="w-full text-left text-xs">
            ...
            <tbody className="divide-y divide-slate-800/60">
              {loading && (
                <tr><td colSpan={5} className="px-5 py-8 text-center text-slate-500">Carregando...</td></tr>
              )}
              {!loading && usuarios.length === 0 && (
                <tr><td colSpan={5} className="px-5 py-8 text-center text-slate-500">Nenhum usuário cadastrado ainda.</td></tr>
              )}
              {usuarios.map((u) => (
                <tr key={u.id} className="hover:bg-slate-800/40 transition">
                  ...
                  <td className="px-5 py-4 text-right">
                    <div className="flex items-center justify-end gap-2">
                      ...
                      <button onClick={() => toggleActive(u)} className="p-2 rounded-xl bg-slate-800 text-rose-400 hover:bg-rose-500 hover:text-white transition" title={u.active ? "Desativar" : "Reativar"}>
