...
//
// O `state` carrega o tenantId codificado em base64: é o único jeito confiável de saber para qual
...
export async function GET(req: NextRequest) {
  ...
  const state = Buffer.from(JSON.stringify({ tenantId, nonce: Math.random().toString(36).slice(2) })).toString(
    "base64url"
  );
