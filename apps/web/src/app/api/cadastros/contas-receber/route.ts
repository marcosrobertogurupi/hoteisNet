...
export async function POST(req: NextRequest) {
  try {
    ...
    const { stayCheckinId, companyId, guestId, billedToName, dueDate, amount, notes } = body;
    ...
    let realCompanyId: string | null = null;
    if (companyId) {
      const company = await prisma.company.findFirst({ where: { id: companyId, tenantId: session.tenantId }, select: { id: true } });
      realCompanyId = company?.id || null;
    }
    let realGuestId: string | null = null;
    if (guestId) {
      const guest = await prisma.guest.findFirst({ where: { id: guestId, tenantId: session.tenantId }, select: { id: true } });
...
export async function PATCH(req: NextRequest) {
  try {
    ...
    const updated = await prisma.accountsReceivable.updateMany({ where: { id, tenantId: session.tenantId }, data });
    if (updated.count === 0) {
      return NextResponse.json({ success: false, error: "Título não encontrado." }, { status: 404 });
    }
    const receivable = await prisma.accountsReceivable.findUnique({ where: { id } });
...
// DELETE /api/cadastros/contas-receber?id=... — exclui um título (apenas se ainda não tiver baixas). Só admin.
export async function DELETE(req: NextRequest) {
  try {
    ...
    const existing = await prisma.accountsReceivable.findFirst({ where: { id, tenantId: session!.tenantId! } });
    if (!existing) {
      return NextResponse.json({ success: false, error: "Título não encontrado." }, { status: 404 });
    }
    ...
    const deleted = await prisma.accountsReceivable.deleteMany({ where: { id, tenantId: session!.tenantId! } });
    if (deleted.count === 0) {
      return NextResponse.json({ success: false, error: "Título não encontrado." }, { status: 404 });
    }
