...
export async function POST(req: NextRequest, { params }: { params: Promise<{ roomId: string }> }) {
  try {
    ...
    // OCCUPIED presa num quarto que já não está ocupado (ver abaixo) não conta como "em andamento".
    const ongoingElsewhere = await prisma.housekeepingTask.findFirst({
      where: {
        ...
        status: "IN_PROGRESS",
        roomId: { not: roomId },
        NOT: { type: "OCCUPIED", room: { status: { not: "OCCUPIED" } } },
      },
    ...
    // Espelha a seleção de "tarefa ativa" de GET /api/housekeeping/rooms (mesma janela de
    // serviceDate e mesmo taskFitsRoomStatus): uma tarefa que não combina com a situação atual do
    // quarto — ex.: arrumação OCCUPIED que sobrou depois do check-out — perdeu o sentido. Sem
    // ignorá-la aqui, o "start" reencontrava a tarefa presa, via que já estava IN_PROGRESS e
    // devolvia sucesso sem transicionar nada — o app recarregava, continuava mostrando a limpeza
    // pós check-out como PENDING e o botão voltava para "Iniciar Limpeza" a cada toque.
    ...
    const openTasks = await prisma.housekeepingTask.findMany({
      where: {
        ...
        OR: [{ serviceDate: null }, { serviceDate: today }],
      },
      orderBy: { createdAt: "desc" },
      select: { id: true, type: true, status: true, housekeeperId: true },
    });
    const existing = openTasks.find((t) => taskFitsRoomStatus(t, room.status)) || null;
    ...
    // Quarto livre e limpo (ou em manutenção) não tem o que limpar — a lista da governanta estava
    // desatualizada. Sem isto, criava-se uma limpeza pós check-out para um quarto já limpo.
    if (!isOccupied && room.status !== "VACANT_DIRTY") {
      return NextResponse.json(
        { success: false, error: "Este quarto não precisa de limpeza no momento — atualize a lista." },
        { status: 409 }
      );
    }
