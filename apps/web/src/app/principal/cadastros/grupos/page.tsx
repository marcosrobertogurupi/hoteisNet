...
const EMPTY_FORM = { id: "", codigo: "", nome: "", tipo: "PRODUTO" as Grupo["type"], active: true };
...
export default function GruposPage() {
  ...
  const handleOpenEdit = (g: Grupo) => {
    setForm({ id: g.id, codigo: g.code || "", nome: g.name, tipo: g.type, active: g.active });
    setIsModalOpen(true);
  };
    ...
    <div className={c.page(theme.bgApp, theme.textMain)}>
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        <div className="flex items-center justify-between">
          <Link href="/principal/cadastros" className={c.backLink}>
            ...
            <table className="w-full text-left text-xs">
              <thead className={c.thead}>
                <tr>
                  ...
                  <th className="px-5 py-3.5 text-center">Em uso</th>
              ...
              <tbody className={`divide-y ${c.tdivide}`}>
                {grupos.map((g) => (
                  <tr key={g.id} className={`transition ${c.rowHover}`}>
                    ...
                    <td className={`px-5 py-4 text-center font-mono ${c.muted}`}>{g.emUso}</td>
                ...
                {grupos.length === 0 && !isLoading && (
                  <tr>
                    <td colSpan={6} className={`px-5 py-12 text-center ${c.empty}`}>
          ...
          <div className={`${c.modalCard} max-w-lg`}>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <label className="flex items-center gap-2 text-xs font-semibold cursor-pointer">
                ...
                Ativo (aparece para seleção nos cadastros)
              </label>
