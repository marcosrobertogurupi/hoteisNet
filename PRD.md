...
* **Visualização de Anexos Recebidos 🟡 (em diagnóstico):** `WhatsappMessage` ganhou os campos `mediaUrl`/`mimeType` para o modal renderizar anexos recebidos do hóspede (preview inline se imagem, link de download nos demais casos). Ainda não está confirmado qual campo do payload da uazapi carrega a URL do anexo — as tentativas `fileURL`/`fileUrl`/`url` vieram vazias nos testes reais; o webhook grava temporariamente o payload bruto (prefixo `[DEBUG-RAW]`) no campo `content` quando isso ocorre, para inspeção via banco até o campo correto ser identificado e o parser corrigido.
