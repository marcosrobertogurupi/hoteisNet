...
Também foi gerada (mas não aplicada em produção) uma migration habilitando Row Level Security nas 61 tabelas do Prisma no Supabase (`supabase/migrations/20260823220000_enable_rls_all_tables.sql`, deny-by-default sem policies) — mitiga o risco de a `NEXT_PUBLIC_SUPABASE_ANON_KEY` (exposta no bundle do frontend) permitir consultas diretas via PostgREST a tabelas sem proteção, ignorando toda a autenticação da aplicação. Aplicar com `supabase db push` fica pendente de decisão.
...
6. **Segunda rodada de auditoria, 23/08/2026 — concluída ✅:** remoção do backend Fastify legado (`apps/api`, órfão e com SSRF real), middleware central protegendo `/api/*` como camada extra, correção de enumeração de contas no login, rate limiting por IP no login, security headers globais, e migration de RLS para as tabelas do Supabase (gerada, aplicação em produção pendente de decisão — ver 2.1). Pendências que **não** entraram nesta rodada: bucket `fnrh-signatures` do Supabase Storage precisa ser confirmado como privado com signed URLs de curta duração; fallback silencioso de `DEFAULT_TENANT_ID` em `verify-admin`/`users` deveria virar rejeição explícita.
